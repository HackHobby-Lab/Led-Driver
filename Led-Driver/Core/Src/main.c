...
const uint8_t spi_encode[4] = {
    0b10001000,  // 00 = two '0' bits
...
//            brightness_at_press = current_brightness;
...
//                            uint8_t neo_bright = (current_brightness * 100) / max_brightness;
//                            neopixel_set_brightness(neo_bright);
//                            neopixel_update();
//                        }
//                        is_on = 1;
//                    }
...
void handle_click_sequence(uint8_t clicks)
{
    ...
    if (is_locked)
    {
        ...
        else if (clicks == 3)
        {
            ...
            // Visual battery check with NeoPixels
            neopixel_set_all(0, 255, 0);  // Green for good battery
            neopixel_set_brightness(50);
            neopixel_update();
            HAL_Delay(500);
            neopixel_set_brightness(0);
            neopixel_update();
...
void smooth_fade_to(uint32_t target)
{
    ...
    for (int i = 1; i <= FADE_STEPS; i++)
    {
        ...
        // Update NeoPixel brightness during fade
        uint8_t neo_bright = (current_brightness * 100) / max_brightness;
        neopixel_set_brightness(neo_bright);
        neopixel_update();
